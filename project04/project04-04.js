...
function runTheRegister() {
   ...
   if (changeValue < 0) {
       document.getElementById("warning").innerHTML = "Cash is less than the bill!";
       document.getElementById("warning").style.color = "red";
       changeBox.value = "0.00";
       return;
   }
...
function calcChange(changeValue) {
   ...
   changeValue -=  bill5Amt*5;  // Fix: should be *5, not *3
   ...
   let coin25Amt = determineCoin(changeValue*100, 25);  // multiplying by 100 for coins
   document.getElementById("coin25").innerHTML = coin25Amt;
   changeValue -= coin25Amt*0.25;   // Fix: multiplying by 0.25 for coin values
   ...
   let coin1Amt = Math.round(changeValue*100);  // rounding pennies
...
function determineCoin(cashValue, currencyUnit) {
   return Math.floor(cashValue / currencyUnit);  // Using Math.floor to round down
}

// Function to display a numeric value as a text string in the format ##.##
function formatCurrency(value) {
   return value.toFixed(2);  // format with two decimals
}
